...
if(!lol) return message.channel.send(new Discord.MessageEmbed().setDescription(`Anime adı belirtmelisin.`).setFooter(`C' Academy`).setColor("RANDOM").setTimestamp());
anime.searchAnime(lol).then(res => {
if(!res) return message.channel.send(new Discord.MessageEmbed().setDescription(`Böyle Bir anime bulamadım.`).setFooter(`C' Academy`).setColor("RANDOM").setTimestamp());
