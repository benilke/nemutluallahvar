...
client.on("message", message => {
    if(message.content.toLowerCase() == "zaur") 
    return message.channel.send(`Zaur adamdır :heart: :heart:`)
});

client.on("message", message => {
  if(message.content.toLowerCase() == "ilke") 
  return message.channel.send(`https://youtu.be/o17AgbomJag <:yargi:789235647570509835>`)
});

client.on("message", message => {
  if(message.content.toLowerCase() == "yasin") 
  return message.channel.send(`https://youtu.be/9gzbHgBGff8 <:yargi:789235647570509835>`)
});

client.on("message", message => {
  if(message.content.toLowerCase() == "yasinbey") 
  return message.channel.send(`https://youtu.be/9gzbHgBGff8 <:yargi:789235647570509835>`)
});
...
let rol = member.guild.roles.cache.find(r => r.name === "791362572065832980");
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('791362572065832980');
  }, ms(sürejail));
}
})*/
...
client.on("message", async msg => {
 ...
 const i = await db.fetch(`kufur_${msg.guild.id}`)
    if (i == "acik") {
        const kufur = [ "allahi sikiyim","anneni","allahini","cc","cardable","sehitleri","ataturku sikiyim","napim","xapim","nabim","allahını","atatürkünü","CC","Cc","allahı"];
...
///photochat
client.on("message", m => {
  if (m.channel.id !== "799599690601463818") { //buraya o kanalın ID'si yazılacak.
    return;
  }
...
/*photochat
client.on("message", m => {
  if (m.channel.id !== "") { //buraya o kanalın ID'si yazılacak.
    return;
  }
  if (m.author.id === m.guild.ownerID) return;
  if (m.attachments.size < 1) {
...
//MOD LOG BAŞLANGIÇ
const botadi = "Academy"
  let modlogkanal = "791723228212756511"
...
client.on('guildBanAdd', async (guild, user) => {
    ...
    .setTimestamp()
    client.channels.cache.get(modlogkanal).send(embed)
});
...
let kanal = client.channels.cache.get('789213034748313642');
kanal.send(`${member.user.tag} üyesi sunucumuza boost bastı!`);
member.send(`${member.guild.name} sunucumuza boost  bastığın için teşekkürler! 2 Boost bastıysan rol için ilke#1001 adlı kişiye ulaş.`);
});// codare ♥
